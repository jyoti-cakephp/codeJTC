                            ...
                            <div class="well well2 clearfix">
                               <?php if(!empty($productImage)){ ?> 
                                ...
                                <div class="col-md-3 text-left">
                                    <br/>
                                    <?php echo substr($result['ProductImage']['image'],0,20);?>
                                </div>
                                    <div class="col-md-2 text-right">
                                    <br/>
                                    <input type="radio" name="data[ProductImage][cover_image]" value="<?php echo $result['ProductImage']['cover_image'];?>" <?php if($result['ProductImage']['cover_image']== 1){ echo 'checked="checked"'; }?> onclick="getCover(<?php echo $result['ProductImage']['id'];?>)"title="Cover Image">
                                </div>
                                    <div class="col-md-2 text-right">
                                    <br/>
                                    <?php echo $this->Html->link('<i class="glyphicon glyphicon-trash"></i>',
                                                array(
                                                    'controller' => 'products',
                                                    'action' => 'admin_deleteImage',
                                                    $result['ProductImage']['id'],$result['ProductImage']['product_id']
                                               ), array(
                                                    'escape' => false,
                                                   'alt' =>'delete',
                                                   'title'=>'delete',
