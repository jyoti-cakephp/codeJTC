...
<div class="row">
                          ...
                          <div class="col-md-3">
                              <select style="width: 210px;" name="data[Product][super_id]" id="superid" onchange="getcatname();">
                                  ...
                                  <?php foreach ($super as $key => $val) { ?>
                                                     <option value="<?php echo $key; ?>"<?php echo ($userDetail['Product']['super_id'] == $key) ? "selected" : "" ?> ><?php echo $val; ?></option>
                                  <?php } ?>
  ...
  <div id="catList">
           ...
           <option value="<?php echo $userDetail['Product']['category_id']; ?>"><?php echo $cat['Category']['category_name'];?></option>
...
<?php echo $this->Form->textarea('description', array('class' => 'ckeditor', 'id' => 'noticeMessage', 'name' => 'data[Product][description]', 'value' => $userDetail['Product']['description'])); ?>
                  ...
                  <!--  <label class="col-md-3 control-label"> Upload Images :</label>
                    <div class="col-md-3">
                        <span id="add" class="btn btn-file" > <input style=" margin-top:0px; " id="upload_image" class="image" type="file" name="data[ProductImage][image][]" multiple="true">
                            <div id="error" style="color:red;"></div>
                            <?php //foreach ($userDetail['ProductImage'] as $result) { ?>
    <?php //echo $this->Html->image('product1/50_' . $result['image']); ?>
<?php //} ?>
                        </span>
                        </br><label class="error" for="image" style="margin-left:2%;"></label>
                    </div> -->
                                ...
                                dd(document).ready(function() {
                                    var validator = dd("#faqs").validate({
                                        ...
                                        messages: {
                                            ...
                                            "data[Product][product_type]": {
                                                required: "Product sizes Should not be blank."
...
<script type="text/javascript">
    ...
    function getsubname() {
        ...
        //alert(cid);
        $.ajax({
            url: SITEPATH + "admin/products/addSub",
            data: {
                cid: cid,
            },
            type: "POST",
     ...
     function getcatname() {
        ...
        //alert(sid);
        $.ajax({
            url: SITEPATH + "admin/products/addCat",
            data: {
                sid: sid,
            },
            type: "POST",
