...
class UsersController extends AppController {
    ...
    function beforeFilter() {
        $this->Auth->allow('register', 'checkEmail', 'login', 'resetPassword', 'setForgotPassword', 'signout','userAccount','changePassword','addressBook','addAddress','deleteAddress','updateAddress','myOrder','viewOrder','popup');
    }
    ...
    function admin_userListing($value=NULL,$value1=NULL) {
        ...
        $conditions = array();
        $conditions[] = array('User.user_type !=' => 'admin');
        if ($name != "") {
            $conditions[] = array('user LIKE' => "%" . $name . "%",'User.user_type !=' => 'admin');
        }
         if ($email != "") {
            $conditions[] = array('User.email' =>$email,'User.user_type !=' => 'admin');
        }
         if ($country != "") {
            $conditions[] = array('User.country' =>$country,'User.user_type !=' => 'admin');
        }
     ...
     //pr($order);die;
        ...
        $count = $this->User->find('count',array('conditions'=>array('User.user_type !=' => 'admin')));
        $this->set(compact('userList','countryList','value','value1','count'));
    }
    ...
    function admin_popUp(){
      $this->layout = "admin";  
       $result = $this->User->find('first', array('conditions' => array('User.user_type' => 'admin')));
       //pr($userDetail);die;
       $this->set(compact('result'));
    }
    
    function admin_activepop() {
        if ($this->User->updateAll(array('User.pop_up' => 0), array('User.user_type' =>'admin'))) {
            $this->Session->setFlash('POP-UP has been deactivated');
            $this->redirect(array('controller' => 'Users', 'action' => 'admin_popUp'));
        }
    }

    function admin_deactivepop() {
        if ($this->User->updateAll(array('User.pop_up' => 1), array('User.user_type' =>'admin'))) {
             $this->Session->setFlash('POP-UP has been activated');
            $this->redirect(array('controller' => 'Users', 'action' => 'admin_popUp'));
        }
    }
      ...
      function myOrder()
    {
     ...
     function viewOrder($id = NULL) {
      ...
      function popup() {
        $this->autoRender = FALSE;
        $popup = $this->User->find('first', array('conditions' => array('User.user_type' => 'admin')));
        return $popup;
    }
}
